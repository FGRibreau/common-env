...
module.exports = function envFactory() {
  ...
  function getOrElseConfigurationObject(fullKeyName, object) {
    if ((object.$type || object.$secure) && _.isUndefined(object.$aliases)) {
      object.$aliases = [];
    }
    if (_.isUndefined(object.$secure)) {
      object.$secure = false;
    }
    if (!Array.isArray(object.$aliases) && typeof(object.$secure) !== 'boolean') {
      throw new Error('Common-env: $aliases or $secure must be defined along side $default, key: ' + fullKeyName);
    }
    ...
    return object.$aliases.concat([fullKeyName]).reduce(function (memo, varEnvName, i, aliases) {
      ...
      if (isLast) {
        return _.isUndefined(object.$default) ? getOrDieWithAliases(varEnvName, $typeConverter, aliases) : getOrElse(varEnvName, object.$default, $typeConverter, object.$secure);
      }

      return getOrElse(varEnvName, null, $typeConverter, object.$secure);
    }, null);
  }
   ...
   * @return {B}
   */
  function getOrElse(fullKeyName, $default, $typeConverter, $secure) {
